...
router.get('/', (req, res) => {
  ...
  const yearsResult = db.prepare("SELECT DISTINCT strftime('%Y', date_vente) as year FROM ventes ORDER BY year DESC").all();
  const availableYears = yearsResult.map(y => y.year).filter(y => y !== null);
